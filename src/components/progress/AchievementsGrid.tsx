...
export const AchievementsGrid = ({ achievements, consecutiveDays, achievedWishes, totalSessions, longestStreak }: AchievementsGridProps) => {
  ...
  return (
    <Card className="border-0 shadow-storybook bg-card overflow-hidden rounded-storybook-lg">
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {badges.map((badge) => (
            <div key={badge.id} className="text-center group">
              <div className={`relative w-14 h-14 rounded-storybook-lg mx-auto mb-2 flex items-center justify-center transition-all duration-300 ${
                badge.unlocked ? `bg-gradient-to-br ${badge.gradient} shadow-storybook` : 'bg-muted'
              }`}>
                {badge.unlocked ? (
              ...
              <div className={`text-xs font-medium ${badge.unlocked ? 'text-foreground' : 'text-muted-foreground'}`}>{badge.name}</div>
            </div>
          ))}
        </div>
      </div>
